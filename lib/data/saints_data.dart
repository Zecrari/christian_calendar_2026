...
class SaintsDatabase {
  static SaintModel getSaint(int month, int day, String lang) {
    ...
    if (_majorFeastIDs.containsKey(key)) {
      ...
      final dataMap = (lang == 'ta') ? _majorSaintsTa : _majorSaintsEn;
    ...
    if (lang == 'ta' && _dailyNamesTa.containsKey(key)) {
      // Case A: We have the Tamil Name
      name = _dailyNamesTa[key]!;
    } else if (_dailyNamesEn.containsKey(key)) {
      // Case B: We only have the English Name (Fallback)
      name = _dailyNamesEn[key]!;
    } else {
      // Case C: We have no data for this date
      name = (lang == 'ta') ? "இன்றைய புனிதர்" : "Saint of the Day";
    }
  ...
  static SaintModel _generateDailySaint(String name, String lang) {
    if (lang == 'ta') {
      return SaintModel(
        ...
        prayer:
            "இறைவா, $name அவர்களின் பரிந்துரையால் எங்கள் அன்றாட வாழ்வில் உமது சாட்சிகளாக வாழ அருள்புரியும். ஆமென்.",
        isMajorFeast: false,
      );
    } else {
      return SaintModel(
